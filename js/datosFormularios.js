...
function abrirEditarParque(idParque) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      document.querySelector("#tituloParque").textContent =
        "Editar parque : " + datosParque.nombre; // Ac
      ...
      document.querySelector("#tipo_atraccion").value =
        datosParque.id_atraccion;
      document.querySelector("#Tipo_Evento").value = datosParque.id_evento;
      document.querySelector("#descripcionParque").value =
        datosParque.descripcion;
    },
...
function AbrirEliminarParque(idParqueEliminado) {
  $.ajax({
    ...
    success: function (response) {
      ...
      // Ejemplo: Actualizar un elemento HTML con los datos recibidos
      var datosParque = JSON.parse(response); // Suponiendo que la respuesta está en formato JSON
      document.querySelector("#parqueEliminar").textContent =
        "Seguro de eliminar el parque : " + datosParque.nombre; // Ac
    },
    error: function (error) {
      // Manejar errores aquí
      console.error(error);
